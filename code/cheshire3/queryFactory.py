...
[bib1 1=5, bib1 3=6] > term and title @ fish"""
    ...
    relations = {'<'        : '<',
                ...
                '.PHON.'    : 'all/phonetic',
                # relevance modifiers
                '@'         : 'all/relevant',                                  # Cheshire3's generic relevance algorithm for result ranking
                'REL'       : 'all/relevant',
                '.REL.'     : 'all/relevant',
                '@@'        : 'all/rel.algorithm=trec2',                       # Berkeley TREC2 algorithm for result ranking
                '.TREC2.'   : 'all/rel.algorithm=trec2',
                '.TREC3.'   : 'all/rel.algorithm=trec3',                       # Berkeley TREC3 Algorithm for result ranking
                '@*'        : 'all/rel.algorithm=trec2/rel.feedback',          # Berkeley TREC2 Algorithm for result ranking with blind relevance feedback
                '.TREC2FBK.': 'all/rel.algorithm=trec2/rel.feedback',  
                '@+'        : 'all/rel.algorithm=okapi',                       # Okapi BM-25 for result ranking
                '.OKAPI.'   : 'all/rel.algorithm=okapi',
                '@/'        : 'all/rel.algorithm=tfidf',                       # TF-IDF for result ranking
                '.TFIDF.'   : 'all/rel.algorithm=tfidf',
                '@&'        : 'all/rel.algorithm=lucene',                      # Lucene Vector Space TFIDF for result ranking
                '.LUCENE.'  : 'all/rel.algorithm=lucene',
                '@#'        : 'all/rel.algorithm=cori',                        # CORI algorithm for result ranking -- N.B. primarily intended for collection summary data (e.g. distributed search)
