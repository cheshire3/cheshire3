...
curResourceMapDOM = newDOMDocument("NewResourceMap");
...
function queryObject(what, selElem) {
  url = "http://www.openannotation.org/shaman/sparql/" + what;
  nf = function() {
...
function createNewItemOptions(tree1, table) {
	...
	if (tree1.selectedNode.nodeDesc) {
	    td.appendChild(createOptions(tree1.selectedNode.nodeDesc));
	} else {
	    td.appendChild(createOptions('ore:Aggregation'));
	}
}

function createExistItemOptions(tree1, table) {
	var tab = document.getElementById(table);
	var tr = document.createElement('tr');
	tab.appendChild(tr);
	var td = document.createElement('td');
