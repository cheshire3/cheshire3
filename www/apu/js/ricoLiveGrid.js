...
Rico.Buffer.Base.prototype = {
  ...
  dom2jstable: function(rowsElement,firstRow) {
    ...
    for ( var i=firstRow || 0; i < trs.length; i++ ) {
      ...
      // next 5 lines added by Cat for filtering to work
     /* if (this.filter == 'true'){
        if (i==firstRow || i==0){
          this.filterOffsetPre=RicoUtil.getContentAsString(cells[0],this.options.isEncoded);
          alert('filterOffsetPre = ' + this.filterOffsetPre)
        } */
       /* if (i==trs.length-1){		
	  this.filterOffsetApp=RicoUtil.getContentAsString(cells[0],this.options.isEncoded);
	    alert('filterOffsetApp = ' + this.filterOffsetApp)
        }*/
     /* }*/
