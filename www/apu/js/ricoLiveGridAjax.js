...
Rico.Buffer.AjaxSQL.prototype = {
  ...
  initialize: function(url,options,ajaxOptions) {
    ...
    //next 6 lines added by Cat for controlling filtering (requires filter=true)
    this.filter='false';
    for (var i=0; i<options.requestParameters.length; i++){
	if (options.requestParameters[i].name == 'filter'){
	    this.filter = options.requestParameters[i].value;
	}
    }
    this.dataSource=url;
...
Rico.Buffer.AjaxSQLMethods.prototype = {
  ...
  registerGrid: function(liveGrid) {
    ...
    this.filterHooks=new Object();
    this.filterHooks[0] = 0;
this.test = 0;
    this.highestHook = 0;
  ...
  fetch: function(offset) {
    ...
    this.timeoutHandler = setTimeout( this.handleTimedOut.bind(this), this.options.bufferTimeout);
    // the if statement (and related else) added by Cat for filter to work - orginal code just content of else
    if (this.filter == 'true'){
	this.sendAjaxRequest(this.getFilterOffset(offset, bufferStartPos),fetchSize,this.ajaxUpdate.bind(this,bufferStartPos));
    }
    else {
  ...
  //function added by Cat for filtering
  getFilterOffset: function(offset, adjustedOffset) {
    //appending
   // if (offset > this.startPos){ 
       if (this.liveGrid.buffer.rows[0]){//this will always be true except very first call          
          if (this.filterHooks[adjustedOffset] == undefined && adjustedOffset > this.highestHook){
              var filterOffset = this.liveGrid.buffer.rows[this.liveGrid.buffer.rows.length-1][0].content;
              this.filterHooks[adjustedOffset] = filterOffset;   
              this.highestHook = filterOffset;
	      Rico.writeDebugMsg(this.filterHooks);
	  }    
	  else {
	     var filterOffset = 0;
	     for (i in this.filterHooks){
                if (offset >= i){
		   //alert('i = ' + i)
		   //alert('adjusted offset = ' + offset)
		   filterOffset = parseInt(this.filterHooks[i]) + parseInt((offset-i));
		   //alert('filter Hooks value = ' + this.filterHooks[i])
		   //alert('filter offset = ' + filterOffset);
		   this.test = parseInt((offset-i))
			alert(this.test)
                }	
	     }	
          }
 ...
 /*   else { 	
	...
	for (i in this.filterHooks){    
            if (offset >= i){
		filterOffset = this.filterHooks[i]/-1  + '.' + (offset-i);
            }	
	}
    }*/
    return filterOffset;
